...
const PLACEHOLDER = (name) =>
  `https://placehold.co/300x300/e8f0fe/2874f0?text=${encodeURIComponent((name || 'P').slice(0, 2).toUpperCase())}`;
...
const ProductCard = (product) => {
  ...
  const imgSrc = !imgError && image && image !== 'default.png'
    ? `/images/${image}`
  ...
  return (
    <Link to={`/product/${productId}`} style={{
      ...
      {/* FIXED HEIGHT image box — prevents tall images from stretching card */}
      <div style={{
        width: '100%',
        height: 200,
        flexShrink: 0,
        ...
        {quantity === 0 && (
          <div style={{
            ...
            fontWeight: 700, color: '#ff6161', fontSize: 14
          }}>Out of Stock</div>
        )}
      </div>
      ...
      <div style={{ padding: '12px 14px', flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <span style={{
            ...
            background: '#26a541', color: '#fff',
            fontSize: 12, fontWeight: 700, padding: '2px 8px', borderRadius: 4
          }}>
            <FiStar size={10} fill="#fff" /> 4.2
          </span>
          <span style={{ fontSize: 11, color: '#878787' }}>(128)</span>
        </div>

        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 16, fontWeight: 700 }}>
            ₹{(specialPrice ?? price)?.toFixed(0)}
          </span>
          {discountPct && <>
            <span style={{ fontSize: 12, color: '#878787', textDecoration: 'line-through' }}>
              ₹{price?.toFixed(0)}
            </span>
            <span style={{ fontSize: 12, fontWeight: 600, color: '#26a541' }}>
              {discountPct}% off
            </span>
          </>}
        </div>
        ...
        <button
          ...
          style={{
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            gap: 6, padding: '9px', borderRadius: 4, fontSize: 13,
            fontWeight: 600, marginTop: 'auto',
            background: quantity === 0 ? '#e0e0e0' : '#2874f0',
            color: quantity === 0 ? '#999' : '#fff',
            border: 'none', cursor: quantity === 0 ? 'not-allowed' : 'pointer',
            opacity: adding ? 0.7 : 1, transition: 'opacity 0.15s'
          }}>
